...
<div class="min-h-screen pt-28 pb-20 flex items-center justify-center transition-colors duration-500">
    <div class="w-full max-w-md p-8 rounded-[2rem] shadow-2xl shadow-indigo-500/10 border border-slate-200 dark:border-slate-800 relative overflow-hidden">
        ...
        <form action="<?= base_url('auth/loginProcess') ?>" method="post" class="space-y-5" id="loginForm">
            ...
            <div>
                <div class="flex justify-between items-center mb-2">
                    ...
                    <button type="button" onclick="showNotAvailableMsg('Fitur Lupa Password')" class="text-xs font-bold text-indigo-600 hover:text-indigo-500 hover:underline cursor-pointer focus:outline-none">Lupa Password?</button>
                </div>
                <div class="relative group">
                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400 group-focus-within:text-indigo-500 transition-colors">
        ...
        <div class="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800 text-center text-sm text-slate-500">
            Belum punya akun? 
            <button type="button" onclick="showNotAvailableMsg('Pendaftaran Akun Baru')" class="text-indigo-600 font-bold hover:underline transition-colors focus:outline-none cursor-pointer">Daftar Sekarang</button>
        </div>
    </div>
</div>
...
function showNotAvailableMsg(fitur) {
    const isDarkMode = document.documentElement.classList.contains('dark');
    
    Swal.fire({
        title: 'Harap Maklum',
        html: `Maaf, saat ini <b>${fitur}</b> belum bisa dilakukan.<br><br>Silakan hubungi administrator jika Anda membutuhkan akses.`,
        icon: 'info',
